...
# Dinamik Renk Fonksiyonu (Gelme Oranı sayfası için özel limit, diğer sayfalar için standart limit)
def renk_kurali_dinamik(val, is_gelme_orani=False):
    try:
        ...
        if is_gelme_orani:
            # Gelme Oranı sayfası kuralı: %40 ve üzeri Yeşil, %39 ve altı Kırmızı
            if v >= 0.40:
                return 'color: #10b981; font-weight: bold;'
            else:
                return 'color: #ef4444; font-weight: bold;'
        else:
            # Diğer tüm sayfaların standart kuralı: %100+ Yeşil, %80-%99 Sarı, %79- Kırmızı
            if v >= 1.0:
                return 'color: #10b981; font-weight: bold;'
            elif v >= 0.8:
